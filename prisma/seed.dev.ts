...
async function main() {
  ...
  await prisma.ecosystemActivity.createMany({
    data: [
      { userId: player.id, zoneId: zone.id, type: "player_joined", message: "Demo Player joined Galaxy Gaming Arena." },
      { userId: player.id, type: "achievement", message: "Demo Player unlocked First Session." },
